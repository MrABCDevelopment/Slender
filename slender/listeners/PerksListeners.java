...
import me.dreamdevs.slender.api.events.*;
import me.dreamdevs.slender.game.Arena;
import me.dreamdevs.slender.game.GamePlayer;
import me.dreamdevs.slender.game.Role;
...
public class PerksListeners implements Listener
{
    ...
    public void pickupPageEvent(SlenderSurvivorPickupPageEvent event) {
        ...
        if(SlenderMain.getInstance().getPlayerManager().getPlayer(event.getArena().getSlenderMan()).getEquippedSlenderManPerk().equals(Perks.SLENDERMAN_PERK_PAGES_BELONGINGS)) {
            if(event.getArena().getCollectedPages() < 3) {
                event.getArena().getSlenderMan().removePotionEffect(PotionEffectType.SPEED);
                event.getArena().getSlenderMan().addPotionEffect(new PotionEffect(PotionEffectType.SPEED, Integer.MAX_VALUE, event.getCollectedPage()));
            }
        }
    }
